...
TEST_CASE("isCorrect() recognizes correct expressions")
{
	...
	SECTION("More mixed expressions")
	{
		REQUIRE(isCorrect("((string)[{another (string)}]){third(s)tring}"));
	}
}
...
TEST_CASE("isCorrect() recognizes when an expression is incorrect")
{
	...
	SECTION("Another overlapping example")
	{
		REQUIRE_FALSE(isCorrect("(string{123{str}(abcd))"));
	}
}
